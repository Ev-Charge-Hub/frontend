...
function Header({ onFilterButtonClick, activeFilter, onBookingButtonClick, activeBooking, isAuthenticated, setStationData }) {
  ...
  return (
    <div className='bg-white drop-shadow-lg fixed top-0 left-0 w-full flex items-center justify-between z-10'>
      <div className="container mx-auto px-4 sm:px-6 py-3">
        <div className="flex items-center justify-between">

          <div className='flex items-center'>
            <img src="logo.png" className='w-10 h-10 object-contain' />
            <h1 className='text-black font-semibold text-2xl ml-4 hidden sm:block md:hidden lg:block'>EvChargeHub</h1>
          </div>
          ...
          <div className='flex justify-center'>
            <div className='relative'>
              <input
                ...
                className='bg-gray-100 sm:w-80 md:w-50 lg:w-80 h-10 px-6 rounded-md text-sm focus:outline-none'
        ...
        {/* Mobile menu */}
        {isMenuOpen && (
          <div className="md:hidden pt-4 pb-2 space-y-4 animate-fade-in flex flex-col items-center justify-center">
            <button
              title='Filter'
              className={`w-full rounded-md p-2 flex items-center space-x-2 bg-white`}
              onClick={handleFilterButtonClick}
            >
              <span>Filter</span>
            </button>
            ...
            <button
              title={isAuthenticated ? 'Log out' : 'Log in'}
              className='w-full rounded-md p-2 flex items-center space-x-2'
              onClick={handleAuthButtonClick}
            >
              {isAuthenticated ? (
                <>
